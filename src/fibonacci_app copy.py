...
# encoding:utf-8
"""
    fibonacci series ghenerator with restful interface and xml or json response
    documents
    
    
    the fib series calculation is utilizes a simple memoization dict.
...
#cherrypy is a lower limit than the sys default on the mac. so we 
#restate the limit, which seems to work. need to give that a closer look
#some time
...
#could have and should have used a config file for that ...
#HOST_IP = '127.0.0.1'
#SERVER_PORT = '8080'
...
class WebApp(object):
    ...
    def default(self, *args, **kwargs):
        ...
        results_header['date'] = datetime.now()
        ...
        while 1:
            ...
            fH =  FibonacciHandler(gMemo=self.gH)
            status, data = fH.run(n)
            del fH
